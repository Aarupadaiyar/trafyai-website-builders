    ...
    <footer className="relative bg-background border-t border-border">
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-16 lg:py-20 grid grid-cols-2 md:grid-cols-4 gap-10">
        <div className="col-span-2 md:col-span-2">
          <span className="text-2xl font-display text-foreground">{brand.name}</span>
          <p className="mt-4 max-w-xs text-sm text-muted-foreground leading-relaxed">{brand.statement}</p>
        </div>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-3">
            {footerColumns.explore.map((item) => (
              <li key={item.label}>
                <Link to={item.href} className="text-sm text-foreground/80 hover:text-foreground transition-colors">
                  {item.label}
                </Link>
              </li>
            ))}
        ...
        <div>
          <span className="text-xs uppercase tracking-wider text-muted-foreground">Solutions</span>
          <ul className="mt-4 space-y-3">
