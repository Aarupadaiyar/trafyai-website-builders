...
export function RevealText({
  ...
  useEffect(() => {
    ...
    const lastLetterDelay = (text.length - 1) * letterDelay;
    const lastOverlayDelay = (text.length - 1) * overlayDelay;
    const totalDelay = lastLetterDelay * 1000 + springDuration + (lastOverlayDelay + overlayDuration) * 1000;
    const timer = setTimeout(onComplete, totalDelay + 300);
    return () => clearTimeout(timer);
        ...
        {text.split("").map((letter, index) => (
          ...
          >
            ...
            <motion.span
              ...
              transition={{ opacity: { duration: 0.1 }, backgroundPosition: { duration: 3, ease: "easeInOut" } }}
              style={{
